...
__global__ void frame_distance_kernel(
  ...
  for (int n=0; n<1; n++) {
    ...
    // relSE3 関数で、フレーム i と j の間の相対姿勢を計算し、平行移動 tij と回転 qij を算出
    relSE3(ti, qi, tj, qj, tij, qij);
    ...
    GPU_1D_KERNEL_LOOP(k, ht*wd) {
      ...
      // homogenous coordinates
      // ピクセル座標 (u,v) と深度（逆深度 disps[ix][i][j]）を用いて、カメラ座標系での 3D 点 Xi を計算
      Xi[0] = (u - cx) / fx;
      Xi[1] = (v - cy) / fy;
      Xi[2] = 1;
      Xi[3] = disps[ix][i][j];
      ...
      // transform homogenous point
      // 3D 点をフレーム i からフレーム j に変換
      actSE3(tij, qij, Xi, Xj);

      // 3D 点 Xj を画像座標系に変換  (u, v) との距離を計算  →画像平面上でのピクセル距離
      du = fx * (Xj[0] / Xj[2]) + cx - u;
      dv = fy * (Xj[1] / Xj[2]) + cy - v;
      d = sqrtf(du*du + dv*dv); // distance in pixels

      total[threadIdx.x] += beta;
      

      // 再投影後の深度が MIN_DEPTH より大きい場合のみ、有効と判定
      if (Xj[2] > MIN_DEPTH) {
  ...
  // 有効なピクセルが全体の 75% 未満の場合、距離を 1000.0 に設定（無効
  // それ以外の場合、誤差の平均値（accum / valid）を dist に格納
  if (thread_id == 0) {
